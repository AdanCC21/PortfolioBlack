...
export default function Modal({ children, title, icon, isOpen, setIsOpen, onClose }: Prompts) {
    ...
    return (
        <div className="fixed top-0 left-0 flex items-center justify-center w-screen h-screen py-8 bg-black/80 overflow-y-auto z-100" onClick={()=>{ setIsOpen(false); onClose?.()}}>
            <div className="flex flex-col gap-4 w-4/6 h-9/10 bg-(--neutral) rounded-xl p-4 z-101" onClick={(e)=>{e.stopPropagation();}}>
