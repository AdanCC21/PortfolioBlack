...
export default function AchivementCard({ diff, currentItem, active = false }: Promtps) {
    ...
    return (
        ...
        >
            <div className="md:hidden flex w-full h-7/10 overflow-hidden rounded-xl">
                <img src={images[1]} alt="logro" className={`w-full  object-cover z-11 transition-all ease-in-out duration-250`} />
            </div>
            
            <div className="hidden md:flex w-full h-7/10 overflow-hidden rounded-xl">
            ...
            <div className="flex justify-between items-center w-full">
                ...
                <span className="text-xs md:text-base opacity-75">{currentItem.date}</span>
            </div>
            <p className="text-sm md:text-base">{currentItem.description}</p>
        </li>
    )
}